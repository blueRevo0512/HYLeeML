...
def parse_args():
	...
	config = {
		...
		"n_workers": 8,
		"valid_steps": 1000,        # 更频繁的验证
		"warmup_steps": 2000,       # 增加预热步数
		"save_steps": 5000,         # 更频繁的保存
		"total_steps": 100000,      # 增加总训练步数
	}
	return config
...
def main(
	...
	criterion = AdditiveMarginSoftmax(margin=0.3, scale=32).to(device)
	...
	for step in range(total_steps):
		...
		if (step + 1) % valid_steps == 0:
			...
			if valid_accuracy > best_accuracy:
				...
				print(f"\nStep {step + 1}, New best model found! Valid Loss: {valid_loss:.4f}, Valid Accuracy: {valid_accuracy:.4f}")
